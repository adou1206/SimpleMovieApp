//
//  MovieCreditsListNetworkManagerTests.swift
//  SimpleMovieAppTests
//
...
final class MovieCreditsListNetworkManagerTests: XCTestCase {
    ...
    func test_with_successful_response_response_is_valid() async throws {
        guard let path = Bundle.main.path(forResource: "MovieCreditsList", ofType: "json"),
              let data = FileManager.default.contents(atPath: path) else {
            XCTFail("Failed to get the MovieCreditsList file")
            return
        }
        ...
        let res = try await MovieNetworkManager.shared.request(
            session: session,
            endpoint: .movie_credits(movie_id: 0),
            type: MovieDetailCredits.self
        )

        let staticJSON = try StaticJSONMapper.decode(file: "MovieCreditsList", type: MovieDetailCredits.self)

        XCTAssertEqual(res, staticJSON, "The returned MovieDetailCredits response should be decoded properly")
    }
    ...
    func test_with_successful_response_with_invalid_json_is_invalid() async {
        ...
        do {
            ...
            if error is MovieNetworkManager.NetworkError {
                XCTFail("The error should be a system decoding error (Movie Credits List)")
            }
        }
    }
}
